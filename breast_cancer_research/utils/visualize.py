...
def string_to_rgb(string):
    return [abs(hash(string*idx)) % 256 for idx in range(1,4)]
...
def overlay_mask(tensor_img, tensor_masks, classnames, alpha: float = 0.5):
    ...
    for mask, classname in zip(tensor_masks, classnames):
        ...
        tmp_masked_img = cv2.bitwise_or(rgb_mask, cv2.bitwise_and(rgb_img, rgb_img, mask=np.bitwise_not(rgb_mask[:,:,0].astype(bool)).astype(np.uint8)))
